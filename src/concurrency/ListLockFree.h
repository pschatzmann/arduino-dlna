 ...
 * \note Thread-safety caveats:
 * - Some operations like size() and operator[] may not be perfectly consistent in
...
class ListLockFree {
 public:
  ...
  /**
   * @brief Bidirectional iterator for ListLockFree.
   * @note Iterator stores a pointer to the owning list for boundary checks.
   * @note Iterators are invalidated if the list is modified.
   */
  class Iterator {
   public:
  ...
  /**
   * @brief Swap contents with another list. Not implemented (returns false).
   * @return Always false. Not lock-free.
   */
  ...
  /**
   * @brief Access element at index (O(n)). Not thread-safe under concurrency.
   * Make sure that no other thread modifies the list while using this.
   * @param index Index of element
   * @return Reference to element, or a static dummy value if out of bounds (no exceptions)
   * @warning If index is out of bounds, returns a reference to a static dummy value.
   */
...
protected:
  ...
  /**
   * @brief Iterator is a friend for access to internals.
   */
